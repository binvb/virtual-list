...
import { getStaticMessage, getMessage } from './help/mock'
...
describe('test static item', () => {
    ...
    it('locate', () => {
        ...
        .then(scrollTop => {
            ...
            cy.compareSnapshot('locate')
        })
        // locate but !sourceData.length
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.setSourceData(await getStaticMessage(0)))
        cy.then(() => {
            cy.get<VirtualScrollExpose>('@exposeFn').then(exposeFn => exposeFn.locate(99))
        })
    ...
    it('update item index 10', () => {
        ...
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.setSourceData(await getStaticMessage(100)))
        cy.get('li[data-index=10]').find('div').invoke('text').as('updateTestOldContent')
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.update(10, getStaticMessage(1)[0]))
        cy.get('li[data-index=10]').find('div').invoke('text')
        .then(content => {
            cy.get('@updateTestOldContent').should('not.be', content)
            cy.compareSnapshot('update')
        })
    })
    it('add item', () => {
        ...
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.add(12, await getStaticMessage(1)))
        cy.get('li').eq(14).invoke('attr', 'data-key')
        .then(key => {
            cy.get('@addItemOld13').should('eq', key)
            cy.compareSnapshot('add')
        })
    })
    it('delete item', () => {
        ...
        cy.compareSnapshot('delete')
    })
    it('locate item index 88 and reset 80 length data', () => {
        // current data should be refresh(all in current source data)
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.setSourceData(await getStaticMessage(300)))
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.locate(43))
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.setSourceData(await getStaticMessage(45)))
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => {
            ...
            cy.get('li').each((item, index) => {
                cy.wrap(item).invoke('attr', 'data-key').then(key => {
                    ...
                    cy.compareSnapshot('locate-reset')
                })
            })
        })
    })
    it('set data(locate && update && del)', () => {
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => exposeFn.setSourceData(await getStaticMessage(10000)))
        cy.get<VirtualScrollExpose>('@exposeFn').then(async(exposeFn) => {
            const updateData = await getStaticMessage(1)[0]
            // locate
